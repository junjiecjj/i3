...
+  是从2017年开始接触Linux的，从《鸟哥的Linux私房菜》开始，开始自己装系统玩，然后成为Linux重度用户，再到shellscript写装机自动配置文件，再到阅读Linux内核的书籍，看不太懂，也没动手写源码，只是写一些小的demo。
...
+ 一旦使用了平铺式窗口管理器，操作效率会大幅提高，再也回不去了。
...
+ 堆叠式窗口管理器：最典型的就是windows桌面，以及linux下的gnome，KDE等，也就是一个桌面可以打开多个窗口，窗口之间可以相互重叠，需要利用鼠标拖动窗口手动调整桌面布局。
+ 平铺式窗口管理器(Tiling Windows Manager)：最大化利用桌面空间，比如只打开一个窗口，则这个窗口会铺满整个屏幕，如果再打开一个窗口（2个窗口），则新窗口占桌面的一半，之前的窗口会占用另一半，再打开一个窗口(3个窗口)，则最新的窗口会占用一般的桌面空间，之前的两个平分另一半桌面空间。当然也看窗口布局方式，比如i3WM、Xmonad等就有标签页形式的窗口布局，类似于浏览器的标签页形式的布局。
